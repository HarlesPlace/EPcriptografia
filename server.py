...
@app.route('/')
def indexPage():
    return('<h1> Bem vindo ao servidor! </h1> <hr> <p><a href="./create">Criar usuário</a> </p><br><p><a href="./login">Fazer Login</a></p>')
...
def loginPage():
    ...
    if request.method=="GET":
        if not user_cookie:
        #red = redirect(url_for('logoutPage'))
        #return red, 302
        ...
        if password==user_password:
            user_id = redirect(url_for('logoutPage'))
...
def logoutPage():
    if request.method=="POST":
        ...
        user_cookie = request.cookies.get('user')
        k=make_response (f'<h1>Logado como { user_cookie }</h1><hr><form action="/logout" method="POST"><button type="submit">LogOut</button></form>') 
        return k,200
